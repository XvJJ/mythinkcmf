<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:47:"themes/admin_simpleboot3/admin\index\index.html";i:1522474576;}*/ ?>
<!DOCTYPE html>
...
<head>
    ...
    <?php $submenus=$menus; function getsubmenu($submenus){ if(!(empty($submenus) || (($submenus instanceof \think\Collection || $submenus instanceof \think\Paginator ) && $submenus->isEmpty()))): foreach($submenus as $menu){ ?>
        <li>
            <?php 
                $menu_name=lang($menu['lang']); $menu_name=$menu['lang']==$menu_name?$menu['name']:$menu_name;
             if(empty($menu['items'])){ ?>
    ...
    <div class="navbar navbar-default">
        <div class="container-fluid">
            <div class="navbar-header">
                <a href="<?php echo url('admin/index/index'); ?>" class="navbar-brand" style="min-width: 200px;text-align: center;">域名管理系统</a>
                    ...
                    <li class="light-blue dropdown" style="border-left:none;">
                        <a data-toggle="dropdown" href="#" class="dropdown-toggle">
                            <?php if(isset($admin['avatar']) && $admin['avatar']): ?>
                                <img class="nav-user-photo" width="30" height="30" src="<?php echo cmf_get_user_avatar_url($admin['avatar']); ?>" alt="<?php echo $admin['user_login']; ?>">
                                <?php else: ?>
                                <img class="nav-user-photo" width="30" height="30" src="/themes/admin_simpleboot3/public/assets/images/logo-18.png" alt="<?php echo (isset($admin['user_login']) && ($admin['user_login'] !== '')?$admin['user_login']:''); ?>">
                            <?php endif; ?>
    ...
    <div class="main-container container-fluid">
        <div class="sidebar" id="sidebar">
            <div class="sidebar-shortcuts" id="sidebar-shortcuts">
                <a class="btn btn-sm btn-warning" href="/" title="<?php echo lang('WEBSITE_HOME_PAGE'); ?>" data-toggle="tooltip">
            ...
            -->
                <?php if(cmf_auth_check(cmf_get_current_admin_id(),'user/AdminAsset/index')): ?>
                    <a class="btn btn-sm btn-info" href="javascript:openapp('<?php echo url('user/AdminAsset/index'); ?>','userAdminAssetindex','资源管理',true);" title="资源管理" data-toggle="tooltip">
                    <i class="fa fa-file"></i>
                </a>
                <?php endif; if(cmf_auth_check(cmf_get_current_admin_id(),'admin/Setting/clearcache')): ?>
                    <a class="btn btn-sm btn-danger" href="javascript:openapp('<?php echo url('admin/Setting/clearcache'); ?>','index_clearcache','<?php echo lang('ADMIN_SETTING_CLEARCACHE'); ?>',true);" title="<?php echo lang('ADMIN_SETTING_CLEARCACHE'); ?>" data-toggle="tooltip">
                    <i class="fa fa-trash-o"></i>
                </a>
                <?php endif; if(cmf_auth_check(cmf_get_current_admin_id(),'admin/RecycleBin/index')): ?>
                    <a class="btn btn-sm btn-danger" href="javascript:openapp('<?php echo url('admin/RecycleBin/index'); ?>','index_recycle','回收站',true);" title="回收站" data-toggle="tooltip">
                    <i class="fa fa-recycle"></i>
                </a>
                <?php endif; if(APP_DEBUG): ?>
                    <a class="btn btn-sm btn-default" href="javascript:openapp('<?php echo url('admin/Menu/index'); ?>','index_menu','<?php echo lang('ADMIN_MENU_INDEX'); ?>',true);" title="<?php echo lang('ADMIN_MENU_INDEX'); ?>" data-toggle="tooltip">
        ...
        <div class="main-content">
            <div class="page-content" id="content">
                <iframe src="<?php echo url('Main/index'); ?>" style="width:100%;height: 100%;" frameborder="0" id="appiframe-0" class="appiframe"></iframe>
            </div>
        </div>
    </div>
